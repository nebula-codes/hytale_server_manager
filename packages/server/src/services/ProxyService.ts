...
export class ProxyService {
  ...
  async startProxy(
    ...
    const config: Required<Pick<ProxyNetworkConfig, 'javaPath' | 'bindAddress' | 'bindPort' | 'publicPort' | 'proxySecret'>> & ProxyNetworkConfig = {
      ...
      certificatePath: rawConfig.certificatePath || '',
      privateKeyPath: rawConfig.privateKeyPath || '',
      maxConnections: rawConfig.maxConnections ?? 10000,
      ...
      metricsPort: rawConfig.metricsPort ?? 0,
      ...
      proxyRegion: rawConfig.proxyRegion || '',
  ...
  private async writeProxyConfig(
    ...
    const trustedProxyLines = (config.proxyProtocol?.trustedProxies || [])
      .map((proxy) => `    - "${proxy}"`)
      .join('\n');
    const payload = [
      ...
      `  required: ${config.proxyProtocol?.required ?? false}`,
      `  headerTimeoutSeconds: ${config.proxyProtocol?.headerTimeoutSeconds ?? 5}`,
      ...(trustedProxyLines
        ? ['  trustedProxies:', trustedProxyLines]
        : ['  trustedProxies: []']),
      '',
      'backends:',
      backendsBlock || '  []',
